...
description: 将 Anthropic 风格的技能（含 Python 脚本）翻译为 Vault Operator 原生技能（含在沙箱中运行的 JavaScript 脚本）。当用户要求翻译、转换、移植或导入 Anthropic 技能时使用，例如"把 Anthropic 的 pdf 技能拿来"或"翻译这个 Python 技能"。干运行阶段会把每个 Python import 对照映射表分类；部分或无法映射的情况会在写入任何文件前触发用户确认对话框。
